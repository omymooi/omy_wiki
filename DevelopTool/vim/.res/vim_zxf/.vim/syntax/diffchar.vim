...
function! s:HighlightDiffChar(key, lncol)
	...
	for [line, col] in items(a:lncol)
		...
		for i in range(len(col))
			...
			let mid += [matchadd(t:DChar.dmc[i % cnum], dl . dc, 0)]
		endfor
...
function! s:JumpDiffChar(dir, pos)
	...
	if has_key(t:DChar.hlc[k], l)
		let hlc = map(filter(copy(t:DChar.hlc[k][l]), '!empty(v:val)'),
						\'v:val[a:pos ? 0 : -1]')  
		if !empty(hlc)
			if !a:dir
			...
			if !empty(hlc)
				...
				call cursor(l, c)
				call s:ShowDiffCharPairs(k, l, c, a:pos)
				exec cwin . "wincmd w"
				return
			endif
		endif
	...
	while 1 <= l && l <= line('$')
		if has_key(t:DChar.hlc[k], l)
			let hlc = map(filter(copy(t:DChar.hlc[k][l]),
...
function! s:ShowDiffCharPairs(key, line, col, pos)
	for idx in range(len(t:DChar.hlc[a:key][a:line]))
		if index(t:DChar.hlc[a:key][a:line][idx], a:col) != -1
			break
		endif
	endfor
...
function! s:ResetDiffCharPairs(key)
