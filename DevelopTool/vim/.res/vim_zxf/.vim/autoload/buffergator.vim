...
if !exists("g:buffergator_show_new_tab")
	let g:buffergator_show_new_tab = 0
endif
...
hi filePath ctermfg=White cterm=none
"hi filePath ctermfg=White ctermbg=2
    ...
    function! l:catalog_viewer.activate_viewport() dict
        ...
        elseif l:bfwn >= 0
            ...
            execute(l:bfwn . " wincmd w")
           	...
           	if g:buffergator_show_new_tab 
				execute("tabedit")
				execute("b " . self.bufnum)
			else
				" create viewport
...
function! s:NewTabCatalogViewer()
    ...
    let l:catalog_viewer["jump_map_list"] = []
    let l:catalog_viewer["path_found"] = 0
    let l:catalog_viewer["path_index"] = 0
    let l:catalog_viewer["path_count"] = 0
    ...
    function! l:catalog_viewer.render_buffer() dict
        setlocal modifiable
        let self.jump_map_list= []
        let self.path_count = 0
        let l:cur_tab_num = tabpagenr()
        call self.claim_buffer()
        call self.clear_buffer()
        call self.setup_buffer_syntax()
        ...
        for l:tidx in range(len(self.tab_catalog))
            ...
            for widx in range(len(l:tabinfo))
                let self.path_found = 0
                let self.path_index = 0
                let l:tabbufnum = l:tabinfo[widx]
                let l:tabbufname = bufname(l:tabbufnum)
				"let l:tabpath = expand(l:tabbufname)
            	"call self.append_line(l:tabpath, l:tidx+1, 1)
                "let subline = "[" . s:_format_filled(l:tabbufnum, 3, 1, 0) . "] "
                "let subline = "[" . string(l:tidx+1) . "] "
                let subline = "  "
				...
				"let l:tabpath = l:subline
				"let l:tabpath =  "<" . string(l:tidx+1) . "> " 
				"let l:tabpath =  "<-->  " 
				"let l:tabpath .= expand(l:tabbufname,":e")
				let l:tabpath =  "" 
                let l:tabpath .= fnamemodify(l:tabbufname, ":.:h")
            	"call self.append_line(l:tabpath, l:tidx+1, 1)
            	call self.append_line(l:tabpath, l:tidx+1, 1, 1)
            	"call self.append_line(l:tabpath, 0, 0)

                if self.display_regime == "basename"
                    let l:subline .= s:_format_align_left(fnamemodify(l:tabbufname, ":t"), 30, " ")
                    "let l:subline .= fnamemodify(l:tabbufname, ":p")
                    "let l:subline .= expand(l:tabbufname)
                elseif self.display_regime == "filepath"
                    let l:subline .= fnamemodify(l:tabbufname, ":p")
                elseif self.display_regime == "bufname"
                ...
                call self.append_line(l:subline, l:tidx+1, l:widx+1, 0)
            endfor
        endfor
        let index = 0
        while index < len(self.jump_map_list)
            let self.jump_map[index+1] = self.jump_map_list[index]
            call append(line("$")-1, self.jump_map_list[index].text)
            let index += 1
        endwhile
    ...
    function! l:catalog_viewer.setup_buffer_syntax() dict
        "highlight filePathZouxf cterm=bold ctermfg=yellow ctermbg=red
        highlight filePathZouxf cterm=bold ctermfg=yellow 
        if has("syntax") && !(exists('b:did_syntax'))
            syn match BuffergatorTabPageLine '^TAB PAGE \d\+\:$'
			syn match BuffergatorTabPathLine '^<\d\+> .*'
			syn match BuffergatorTabPathLine '^<--> .*'
			"syn match BuffergatorTabPathLine '^<\d\+>'
            "syn match BuffergatorTabPageLine '^\[\d+\] .*'
            "syn region BuffergatorTabPageLine start='^\[\d+\] + ' keepend oneline end='$'
            ...
            "highlight! link BuffergatorTabPathLine filePath 
            highlight! link BuffergatorTabPathLine filePathZouxf
    ...
    " Appends a line to the buffer and registers it in the line log.
    function! l:catalog_viewer.append_line(text, jump_to_tabnum, jump_to_winnum, is_path) dict
        let l:line_map = {
        ...
        if a:is_path == 1
            let index = 0
            while index < len(self.jump_map_list)
                if self.jump_map_list[index].is_path == 1
                    if self.jump_map_list[index].path == a:text
                        let self.path_found = 1
                        let self.path_index = index
                    endif
                endif
                let index += 1
            endwhile

            if self.path_found == 0
                let self.path_count += 1
                let l:line_map.text = "<" . string(self.path_count) . "> "
                let l:line_map.text .= l:line_map.path
                call add(self.jump_map_list, l:line_map)
            endif
        else
            if self.path_found == 1
                call insert(self.jump_map_list, l:line_map, self.path_index+1)
            else
                call add(self.jump_map_list, l:line_map)
            endif
            let self.path_found = 0
