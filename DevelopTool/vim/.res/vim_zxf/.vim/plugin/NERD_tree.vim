...
call s:initVariable("g:NERDTreeMapToggleFilters", "E")
call s:initVariable("g:NERDTreeMapCommandTCurrentNERDTreeNode", "f")
...
function! s:TreeFileNode.openzxf()
    let currentTab = tabpagenr()
...
function! s:createTreeWin()
    ...
    silent! exec "tabedit " . t:NERDTreeBufName
...
function! s:indentLevelFor(line)
	let newline = substitute(a:line,"─","+","g")
	let newline = substitute(newline,"│","+","g")
	let newline = substitute(newline,"├","+","g")
    "let level = match(a:line, '[^ \-+~▸▾`|]') / s:tree_wid
    let level = match(newline, '[^ \-+~▸▾`|]') / s:tree_wid

	"let tmp = match(newline, '[^ \-+~▸▾`|]') 
	"echo newline
	"echo 'tmp = ' tmp
	"echo 'lev = ' level
...
function! s:CommandTCurrentNERDTreeNode()
    let n = s:TreeFileNode.GetSelected()
    if n != {}
...
function! s:bindMappings()
    ...
    exec "nnoremap <silent> <buffer> ". g:NERDTreeMapCommandTCurrentNERDTreeNode ." :call <SID>CommandTCurrentNERDTreeNode()<cr>"
